    ...
    public int sum(int row, char column, String[] numbers) {
        ...
        for(String number: numbers){
            ...
            if(number.indexOf(":") != -1){
                ...
                for(int row1 = startRow; row1 <= endRow; row1++) {
                    for(char col1 = startCol; col1 <= endCol; col1++){
                        ...
                        Node node1 = nodeMap.get(key1);
                         System.out.println("Iterate  " + row1 + " row " + col1 + " col " +Arrays.toString(numberSplit));
                        System.out.println("Rows1 " + row + " " + column);
                        sum += findSum(node1, new HashMap<>(), new HashSet<>());
                        System.out.println("Rows2 " + row + " " + column + " sum " + sum);
                        tempNodes.add(node1);
                    }
                }
            } else {
                ...
                Node node1 = nodeMap.get(key1);
                System.out.println("Rows11 " + row + " " + column);
                sum += findSum(node1, new HashMap<>(), new HashSet<>());
                System.out.println("Rows22 " + row + " " + column + " sum " + sum);
                tempNodes.add(node1);
            }
        }
