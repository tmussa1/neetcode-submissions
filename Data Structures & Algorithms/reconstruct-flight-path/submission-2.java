class Solution {
    public List<String> findItinerary(List<List<String>> tickets) {
        ...
        dfs(adjList, "JFK", result, tickets.size(), visitedMap);

        return result;
    }

    private boolean dfs(Map<String, List<String>> adjList, String node, List<String> path, int ticketCount, Map<String, Integer> visited){
        ...
        for(String neighbor: adjList.get(node)){
            ...
            if(visited.get(key) > 0){
                ...
                if(dfs(adjList, neighbor, path, ticketCount, visited)) {
                    return true;
                }
                visited.put(key, visited.get(key) + 1);
            }
